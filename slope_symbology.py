...
class AttributeLayerSymbology(QtCore.QObject):
    ...
    PARAMS = {"radius": 10000, "cutfill": 1e12}
    ...
    def __init__(
        ...
        target_field,
        target_field_radius,
        target_field_penalties,
        target_field_cutfill,
        target_field_height_cutfill,
        target_field_cutfill_pen,
        min_allowed_value,
        max_allowed_value,
        min_allowed_radius_value,
    ...
    def _intervalValues(self, slopes=None):
        ...
        bottom_val_p = min(min_slope_p, -self.max_allowed_slope_p * self.top_lim)
        top_val_p = max(max_slope_p, self.max_allowed_slope_p * self.top_lim)
        just_under_min_val_p = -self.max_allowed_slope_p * self.just_over_max_lim
        just_over_max_val_p = self.max_allowed_slope_p * self.just_over_max_lim

        intervals = [bottom_val_p, just_under_min_val_p]
        intervals.extend(
            np.linspace(
        ...
        intervals.extend([just_over_max_val_p, top_val_p])
    ...
    def _intervalValues_radius(self, param):
        intervals = []
        intervals.extend(
            np.linspace(0, 100, self.seminum_intervals + 1, endpoint=True) * param
        )
        return intervals
    ...
    def _colorRange(c_start, c_end, num):
        ...
        greens = np.linspace(c_start.green(), c_end.green(), num, endpoint=True)
        blues = np.linspace(c_start.blue(), c_end.blue(), num, endpoint=True)
        alphas = np.linspace(c_start.alpha(), c_end.alpha(), num, endpoint=True)
    ...
    def _colorValues(self):
        ...
        colors = [self.COLORS["outofbounds_2"], self.COLORS["outofbounds_1"]]
        ...
        colors.extend(
            self._colorRange(
                self.COLORS["zeroplusslope"],
                self.COLORS["maxallowedslope"],
                self.seminum_intervals - 1,
            )
        )

        colors.extend([self.COLORS["outofbounds_1"], self.COLORS["outofbounds_2"]])
        return colors
    ...
    def _colorValues_MinSlope(self):
        ...
        colors.extend(
            self._colorRange(
                self.COLORS["minallowedslope_minusslope"],
        ...
        colors.extend(
            self._colorRange(
                self.COLORS["minallowedslope_zeroplusslope"],
    ...
    def updateSymbology(self):
        if self._layer and self._layer.isValid():
            att_values = [
                f[self.target_field] for f in self._layer.dataProvider().getFeatures()
            ]

            att_values_rad = [
                f[self.target_field_radius]
                for f in self._layer.dataProvider().getFeatures()
            ]
            att_values_cutfill = [
                f[self.target_field_cutfill_pen]
                for f in self._layer.dataProvider().getFeatures()
            ]
            intervs = self._intervalValues_tot(
                att_values,
                att_values_rad,
                att_values_cutfill,
                self.param_rad,
            ...
            for v_min, v_max, col in zip(intervs[:-1], intervs[1:], colors[:-1]):
                ...
                if v_max < self.param_rad / self.seminum_intervals:
                    ...
                    label = (
                        "{:0.2f}%".format(v_lab_min)
                        + "\u2264"
                        + " dif rad < {:0.2f}%".format(v_lab_max)
                    )
                else:
                    v_lab_min_ct = v_min / self.param_cutfill
                    v_lab_max_ct = v_max / self.param_cutfill
                    label = (
                        "{:0.2f}%".format(v_lab_min_ct)
                        + "< dif cut/fill "
                        + "\u2264"
                        + " {:0.2f}%".format(v_lab_max_ct)
                    )
...
class InteractiveAttributeLayerSymbology(QtCore.QObject):
    ...
    COLORS = {
        "simple_color": QtGui.QColor(0, 0, 255),
    }

...
class ForestRoadDesignerProcessingAlgorithm(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        ...
        """
        self.addParameter(
            QgsProcessingParameterRasterLayer(self.INPUT, self.tr("Introduce capa"))
        )
        ...
        self.set_parameters_number(
            self.MIN_SLOPE, "Introduce min_slope", self.PARAM_TYPE_DOUBLE, 0, 10000
        )
        self.set_parameters_number(
            self.MAX_SLOPE,
            "Introduce max_slope",
        ...
        self.set_parameters_number(
            self.MIN_CURVE_RADIO,
            "Introduce radio mínimo de curvatura",
            self.PARAM_TYPE_DOUBLE,
            0,
            10000,
        )
        ...
        self.set_parameters_number(
            self.W_ROAD, "Introduce ancho de pista", self.PARAM_TYPE_DOUBLE, 0, 10000
        )
        self.set_parameters_number(
            self.CUT_ANGLE,
            "Introduce pendiente de desmonte",
            self.PARAM_TYPE_DOUBLE,
            0,
            10000,
        )
        self.set_parameters_number(
            self.CUT_HMAX,
            "Introduce altura máxima de desmonte",
            self.PARAM_TYPE_DOUBLE,
            0,
            10000,
        )
        self.set_parameters_number(
            self.FILL_ANGLE,
            "Introduce pendiente máxima de terraplén",
            self.PARAM_TYPE_DOUBLE,
            0,
            10000,
        )
        self.set_parameters_number(
            self.FILL_HMAX,
            "Introduce altura máxima de terraplén",
            self.PARAM_TYPE_DOUBLE,
            0,
            10000,
        )
        self.set_parameters_number(
            ...
            "Introduce penalización por cambio de dirección",
            self.PARAM_TYPE_INTEGER,
            0,
            100,
            40,
        )
        self.set_parameters_number(
            ...
            "Introduce penalización por cambio de rasante",
            self.PARAM_TYPE_INTEGER,
            0,
            100,
            40,
        ...
        self.set_parameters_number(
            self.SEMI_SIZE,
            "Introduce longitud segmento mínima",
            self.PARAM_TYPE_INTEGER,
            2,
            10,
            2,
        ...
        self.set_parameters_number(
            self.POLILINE_TRESHOLD,
            "Introduce tolerancia al resultado",
            self.PARAM_TYPE_INTEGER,
            0,
            5,
            0,
        ...
        self.set_parameters_number(
            self.SLOPE_PENALTY,
            "Introduce factor penalización de pendiente",
            self.PARAM_TYPE_INTEGER,
            1,
            10,
            1,
        ...
        self.set_parameters_number(
            self.RADIUS_PENALTY,
            "Introduce factor penalización de radio",
            self.PARAM_TYPE_INTEGER,
            1,
            10,
            1,
        ...
        self.set_parameters_number(
            self.CUTFILL_PENALTY,
            "Introduce factor penalización de desmonte/terraplén",
            self.PARAM_TYPE_INTEGER,
            1,
            10,
            1,
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        exclusion_areas_layer = self.parameterAsVectorLayer(
            parameters, self.EXCLUDED_INPUT, context
        )
        ...
        if Path(outputFolder).name == self.OUTPUT_DIR:
            outputFolder = tempfile.mkdtemp("frd")
        poliline_treshold = self.parameterAsInt(
            parameters, self.POLILINE_TRESHOLD, context
        )

        input_parameters["min_slope_pct"] = self.parameterAsDouble(
            parameters, self.MIN_SLOPE, context
        )
        input_parameters["max_slope_pct"] = self.parameterAsDouble(
            parameters, self.MAX_SLOPE, context
        )
        input_parameters["min_curve_radio_m"] = self.parameterAsDouble(
            parameters, self.MIN_CURVE_RADIO, context
        )
        input_parameters["w_road_m"] = self.parameterAsDouble(
            parameters, self.W_ROAD, context
        )
        input_parameters["activated_road_options"] = self.parameterAsBoolean(
            parameters, self.ACTIVATED_ROAD_OPTIONS, context
        )
        input_parameters["cut_angle_tan"] = self.parameterAsDouble(
            parameters, self.CUT_ANGLE, context
        )
        input_parameters["cut_hmax_m"] = self.parameterAsDouble(
            parameters, self.CUT_HMAX, context
        )
        input_parameters["fill_angle_tan"] = self.parameterAsDouble(
            parameters, self.FILL_ANGLE, context
        )
        input_parameters["fill_hmax_m"] = self.parameterAsDouble(
            parameters, self.FILL_HMAX, context
        )
        input_parameters["penalty_factor_xy"] = self.parameterAsInt(
            parameters, self.PENALTY_FACTOR_XY, context
        )
        input_parameters["penalty_factor_z"] = self.parameterAsInt(
            parameters, self.PENALTY_FACTOR_Z, context
        )
        input_parameters["semi_size"] = self.parameterAsInt(
            parameters, self.SEMI_SIZE, context
        )
        input_parameters["slope_penalty_factor"] = self.parameterAsInt(
            parameters, self.SLOPE_PENALTY, context
        )
        input_parameters["radius_penalty_factor"] = self.parameterAsInt(
            parameters, self.RADIUS_PENALTY, context
        )
        input_parameters["cutfill_penalty_factor"] = self.parameterAsInt(
            parameters, self.CUTFILL_PENALTY, context
        )
        ...
        try:
            if input_dtm is None:
                ...
                input_parameters["semi_size"] = (
                    self.parameterAsInt(parameters, self.SEMI_SIZE, context)
                    / dtm_pixel_size
                )

            if input_v_layer is None:
                raise QgsProcessingException(
                    self.invalidSourceError(parameters, self.POLILINE)
                )

            if outputFolder is None:
                raise QgsProcessingException(
                    self.invalidSourceError(parameters, self.OUTPUT_DIR)
                )
            ...
            feedback.pushInfo(
                "exclusion_areas_layer Type is {}".format(type(exclusion_areas_layer))
            )
            feedback.pushInfo("input_v_layer Type is {}".format(type(input_v_layer)))
            feedback.pushInfo(
                "ckeck_road_options {}".format(
                    self.ckeck_road_options(input_parameters)
                )
            )

            feedback.pushInfo(
                "..simplified_layer.isValid() {}".format(simplified_layer.isValid())
            )
            feedback.pushInfo(
                "..featureCount {}".format(raw_layer.dataProvider().featureCount())
            )
            ...
            summary_msg = self.create_sumary_message(
                summary, dtmMapUnit, input_parameters
            )
    ...
    def ckeck_road_options(self, parameters):
        ...
        check = False
        if (
            parameters["w_road_m"] > 0
            and parameters["cut_angle_tan"] > 0
            and parameters["cut_hmax_m"] > 0
            and parameters["fill_angle_tan"] > 0
            and parameters["fill_hmax_m"] > 0
        ):
            check = True

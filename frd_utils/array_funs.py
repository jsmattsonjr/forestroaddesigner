...
def waypoints_list(waypoints_layer):
    ...
    for elem in provider:
        ...
        if (
            ...
            or geom.wkbType() == QgsWkbTypes.MultiLineString
        ):
...
def array_2_raster(dtm_layer, waypoints_layer, output_folder, waypoints_coords_list):
    ...
    data_set_out.SetGeoTransform(
        (
            data_set_origin_x,
            data_set_pixel_width,
            0,
            data_set_origin_y,
            0,
            data_set_pixel_height,
        )
    )
...
class Converter(object):
    def __init__(self, layer):
        ...
        self.x_0_c = self.data_set_geotransform[0] + (self.data_set_geotransform[1] / 2)
        self.y_0_c = self.data_set_geotransform[3] + (self.data_set_geotransform[5] / 2)
    ...
    def coord_to_index(self, waypoints_coords_list):
        ...
        if wpts_coords_array.size > 0:
            wpts_index_array = (
                wpts_coords_array - np.array([[self.x_0_c, self.y_0_c]], dtype=float)
            ) / np.array([[self.pixel_width, self.pixel_height]], dtype=float)
    ...
    def index_to_coord(self, waypoints_index_array):
        ...
        if len(waypoints_index_array) > 0:
            waypoints_coords_array = np.array(
                _swap_xy(waypoints_index_array), dtype=float
            ) * np.array(
                [[self.pixel_width, self.pixel_height]], dtype=float
            ) + np.array(
                [[self.x_0_c, self.y_0_c]], dtype=float
            )
...
def coord_to_index(dtm_layer, waypoints_coords_list):
    ...
    wpts_index_array = (
        wpts_coords_array - np.array([[x_0_c, y_0_c]], dtype=float)
...
def index_to_coord(data_set_geotransform, waypoints_index_array):
    ...
    waypoints_coords_array = np.array(
        _swap_xy(waypoints_index_array), dtype=float
    ) * np.array([[pixel_width, pixel_height]], dtype=float) + np.array(
        [[x_0_c, y_0_c]], dtype=float
    )
    return waypoints_coords_array


def point_layer_from_coords_array(
    waypoints_coords_array, output_folder, crs, slope_label, points_height_data
):
    ...
    key_for_glob = os.path.join(
        output_folder, road_points_mask.format(slope_label, "*")
    )
    ...
    writer_error = writeFileAsVectorFormatV3(
        forest_road_points_mem_layer, points_out_path, transform_context, save_options
    )
...
def line_layer_from_coords_array(
    ...
    forest_road_lines_mem_layer = QgsVectorLayer(
        "LineString?crs={}".format(crs.toWkt()), "forest_road", "memory"
    )

    provider = forest_road_lines_mem_layer.dataProvider()
    distances = []
    for point_0, point_1 in zip(
        waypoints_coords_array[:-1], waypoints_coords_array[1:]
    ):
    ...
    key_for_glob = os.path.join(output_folder, file_mask.format(slope_label, "*"))
    files_list = glob.glob(key_for_glob)
    ...
    transform_context = QgsProject.instance().transformContext()
    writer_error = writeFileAsVectorFormatV3(
        forest_road_lines_mem_layer, lines_out_path, transform_context, save_options
    )

...
def initialize_field(layer, field_name, values, variant, check_field_input=False):
    ...
    if variant == "double":
        layer.dataProvider().addAttributes(
            [QgsField(field_name, QVariant.Double, "", 20, 3)]
        )
    if variant == "str":
...
def create_fields_for_points(points_layer, waypoints_coords_array, height_data):
...
def create_fields_for_lines(
    ...
    new_fields["real_d_m"] = np.sqrt(
        (np.array(new_fields["arri_x"]) - np.array(new_fields["start_x"])) ** 2
        + (np.array(new_fields["arri_y"]) - np.array(new_fields["start_y"])) ** 2
        + (np.array(new_fields["arri_z"]) - np.array(new_fields["start_z"])) ** 2
    )
    new_fields["cum_re_m"] = np.cumsum(np.array(new_fields["real_d_m"]))
    new_fields["slope_p"] = [s * 100 for s in lines_section_slope[1:]]
    new_fields["slope_d"] = [np.rad2deg(np.arctan(s)) for s in lines_section_slope[1:]]
...
def interactive_create_fields_for_lines(
    layer,
    height_data,
    waypoints_coords_array,
    lines_section_slope,
    ...
    new_fields["slope_d"] = [np.rad2deg(np.arctan(s)) for s in lines_section_slope[1:]]
    new_fields["z_var"] = [
        (h1 - h0) for h0, h1 in zip(height_data[:-1], height_data[1:])
    ]
    # new_fields['z_var'] = np.diff(height_data)

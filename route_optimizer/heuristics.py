...
class PrecalculatedOptimalDistanceSlopeConstrained(object):
    def __init__(
        self, max_dist, dtm_array, max_slope, scale_m_per_pix, target_scale=None
    ):
        ...
        self.scaled_size = [int(s * self.target_scale) for s in dtm_array.shape]

        self.dtm_array_scaled = zoom(
            dtm_array,
            self.target_scale,
            output=None,
            order=3,
    ...
    def retarget(self, point):
        ...
        self.distance_transformed_dtm = zoom(
            distance_transformed_dtm_scaled,
...
class DistanceMinMaxSlopeMetric(object):
    # Checks slopes in
    def __init__(self, array, max_slope, max_dist, min_slope=-1):
    ...
    def __call__(self, a, b, dist):
        ...
        # while approaching the solution
            ...
            dist += self.max_dist * (1 + abs(abs(local_slope) - self.min_slope) ** 2)
        else:
            dist += self.max_dist * (1 + abs(abs(local_slope) - self.max_slope) ** 2)

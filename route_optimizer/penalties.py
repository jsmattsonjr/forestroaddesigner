        ...
        for c_to, d_to, _, _ in zip(*neighbors):
            ...
            angle = np.arccos(
                min(
                    max(
                        np.dot(c_from, c_to)
                        / (np.linalg.norm(c_from) * np.linalg.norm(c_to)),
                        -1,
                    ),
                    1,
                )
            ...
            if self._min_curve_radius is not None:
                rad_pen = (
                    (self._min_curve_radius - 1 / (np.tan(angle / 2)))
                    if (angle != 0 and 1 / (np.tan(angle / 2)) < self._min_curve_radius)
                    else 0
                )
...
class ParabolicPenalty(object):
    ...
    def __init__(
        ...
        penalty_radius,
    ):  # Optional for min curvature radius
    ...
    def __call__(self, current, idx_from, idx_to, slope_from, slope_to):
...
class ParabolicPenaltyCutFill(object):
    ...
    def _cutfill_penalty_idx_to(self, current, idx_to):
        ...
        for orto in orto_list:
            ...
            pen_orto = 0
            if (
                0 <= orto_real[0] < self.array.shape[0]
                and 0 <= orto_real[1] < self.array.shape[1]
            ):
                h_orto_cut = self.cut_angle * (
                    abs(np.linalg.norm(orto) - self.w_road / 2)
                )
                h_orto_fill = self.fill_angle * (
                    abs(np.linalg.norm(orto) - self.w_road / 2)
                )
                hmax = min(self.cut_hmax, h_orto_cut)
                hmin = min(self.fill_hmax, h_orto_fill)
                h_dif = (
                    self.array[orto_real[0], orto_real[1]]
                    - self.array[current[0], current[1]]
                )
                if 0 <= h_dif < hmax or 0 >= h_dif > -hmin:
                    pass
                else:
                    pen_max = (
                        self._penalty_cutfill * abs(1 - h_dif / hmax) ** 2
                        if h_dif > 0
                        else 0
                    )
                    pen_min = (
                        self._penalty_cutfill * abs(1 - abs(h_dif / hmin)) ** 2
                        if h_dif < 0
                        else 0
                    )
...
class PenaltyStatic:
    ...
    def __init__(self, static_penalty_map, dynamic_penalty):

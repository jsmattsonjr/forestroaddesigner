...
def get_interpol(array):
    ...
    interpolator = scipy.interpolate.RegularGridInterpolator(
        (coords_y, coords_x), array, bounds_error=False
    )
    return interpolator
...
class InteractiveDefaultConstraintsOptimizer(object):
    ...
    def interactive_add_segment(self, start, goal):
        ...
        circle_x, circle_y, alpha_rad = self.candidates(
            center_x, center_y, self.array_resolution_m_per_pix, self.R0_px
        )
        # self._last_passing_point.append(current_passing_point)
        # print(f"MODI INTERACTIVO interactive_add_segment ALPHA_0 {alpha_0}")

        dist = self.find_roots(
            self.interpolator,
            alpha_rad,
            circle_x,
            circle_y,
    ...
    def interactive_search_segment_to_(self, goal):
        ...
        if self._waypoints_index:
            if self._waypoints_index[-1][-1] == goal:
                ...
                goal, up_down = self.interactive_search_segment(
                    self._waypoints_index[-1][-1], goal
                )
        return goal, up_down
    ...
    def interactive_search_segment(self, start, goal):
        ...
        if len(dist) > 0:
            ...
            return [point_x, point_y], self.up_down_segment(
                point_z_interp, poinz_z_center
            )
        else:
            return None, None
    ...
    def candidates(self, center_x, center_y, dtm_m, R0_px):
        ...
        alpha_rad = np.linspace(
            0, 2 * np.pi, int(N) + 1, endpoint=True
        )  # Include endpoint to find roots in the interval [360º-360º/N -> 360º]
    ...
    def find_roots(
        ...
        for d_z_m in deltas:
            ...
            circle_func = self.periodic(
                scipy.interpolate.interp1d(
                    alpha, circle_z - (z_center + d_z_m), fill_value="extrapolate"
                )
            )
            ...
            try:
                ...
                print(
                    "Error: no solution at given slope & altitude change for this map!!!!"
                )
                dists = []
        ...
        dists = [
            [(np.cos(alpha) - x_0) ** 2 + (np.sin(alpha) - y_0) ** 2, alpha, d_z_m]
            for alpha, d_z_m in alphas
        ]
        # print(dists)

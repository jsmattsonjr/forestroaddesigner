...
PREDET_PENALTY_FACT = {"slope": 100, "radius": 10000, "cutfill": 1e12}
...
def height_profile(dtm_m, waypoints, min_slope, max_slope, scale_m_per_pix):
    ...
    dist_xy = [
        scale_m_per_pix * np.linalg.norm(p1 - p0)
    ...
    slope_pen_max = [
        (
            100 * abs(slo - max_slope) / abs(max_slope)
            if (max_slope < slo or -max_slope > slo)
            else 0
        )
        for slo in slope
    ]
    ...
    slope_pen_min = [
        (
            100 * (1 - abs(slo - min_slope) / abs(min_slope))
            if (slo > -min_slope and slo < min_slope)
            else 0
        )
        for slo in slope
    ]
...
def radius_profile(dtm_m, waypoints, scale_m_per_pix, min_radius):
    ...
    for p0, p1, p2 in zip(waypoints[:-2], waypoints[1:-1], waypoints[2:]):
        ...
        if angle > 1e-06:
            rad = (
                scale_m_per_pix * 0.5 * suma_arc / angle
                if angle < np.pi
                else 94906265.2485031
            )
        else:
            rad = 0
    ...
    for w_point, o_point_list in zip(waypoints[:-1], orto_points):
        ...
        for o_point_rel in o_point_list:
            ...
            if 0 <= o_point[0] < dtm_m.shape[0] and 0 <= o_point[1] < dtm_m.shape[1]:
                h_orto_cut = cut_angle * abs(
                    np.linalg.norm(o_point - w_point) * scale_m_per_pix - w_road / 2
                )
                h_orto_fill = fill_angle * abs(
                    np.linalg.norm(o_point - w_point) * scale_m_per_pix - w_road / 2
                )
                hmax = min(cut_hmax, h_orto_cut)
                hmin = min(fill_hmax, h_orto_fill)
                h_dif_orto = (
                    dtm_m[o_point[0], o_point[1]] - dtm_m[w_point[0], w_point[1]]
                )
                if 0 <= h_dif_orto < hmax or 0 >= h_dif_orto > -hmin:
                    cutfill_tag_orto = max(
                        abs(h_dif_orto - hmax)
                        / abs(np.linalg.norm(o_point - w_point) - w_road / 2),
                        abs(h_dif_orto - hmin)
                        / abs(np.linalg.norm(o_point - w_point) - w_road / 2),
                    )
                    pass
                    ...
                    pen_dif_min = (
                        abs(1 - abs(h_dif_orto / hmin)) if h_dif_orto < -1e-2 else 0
                    )
                    pen_orto = max(pen_dif_max, pen_dif_min)
                    cutfill_tag_orto = (
                        abs(h_dif_orto - hmax)
                        / (np.linalg.norm(o_point - w_point) * scale_m_per_pix)
                        if pen_dif_max >= pen_dif_min
                        else abs(h_dif_orto - hmin)
                        / (np.linalg.norm(o_point - w_point) * scale_m_per_pix)
                    )
        ...
        cutfill_tag = (
            cutfill_tag_orto_list[0]
            if pen_orto_list[0] >= pen_orto_list[1]
            else cutfill_tag_orto_list[1]
        )
...
def penalty_profile(
    ...
    height_prof, slope, cumdist_xy, slope_pen = height_profile(
        dtm_m, waypoints, min_slope, max_slope, scale_m_per_pix
    )
    ...
    row_rad, row_rad_pen = radius_profile(
        dtm_m, waypoints, scale_m_per_pix, min_radius_profile
    )

    height_diff_prof, cutfill_tan_prof, cutfill_pen_prof = cutfill_profile(
        dtm_m,
        waypoints,
        scale_m_per_pix,
        activated_road_options,
    ...
    penalty_slope_radius_cutfill = [
        pen_sl_rd(
            ...
            PREDET_PENALTY_FACT["cutfill"],
        )
        for sl, sl_pen, rd_pen, rd_pen_prev, cutfill_pen in zip(
            slope[1:],
            slope_pen[1:],
            row_rad_pen[:-1],
            row_rad_pen_prev[:-1],
...
def summary(
    ...
    summary["straight_distance"] = scale_m_per_pix * np.linalg.norm(
    ...
    real_d_m = np.sqrt(
        ((np.array(w_arri_x) - np.array(w_start_x)) * scale_m_per_pix) ** 2
        + ((np.array(w_arri_y) - np.array(w_start_y)) * scale_m_per_pix) ** 2
        + (np.array(w_arri_z) - np.array(w_start_z)) ** 2
    )
    ...
    summary["average_slope"] = (
        abs(height_prof[-1] - height_prof[0]) / summary["straight_distance"]
    )
    summary["average_acum_slope"] = (
        summary["total_acumulative_slope"] / summary["total_cumsum"]
    )
    ...
    track_quality, twist_num, turn_number_list = get_track_quality(
        waypoints, height_prof, scale_m_per_pix
    )
...
def get_track_quality(waypointlist, height_prof, scale_m_per_pix):
    ...
    for i in range(0, len(curve_index) - 1):
        ...
        if abs(suma_ang) >= 30:
            if curve_index[i + 1] - curve_index[i] == 1 and curve_index[
                i + 1
            ] + 1 < len(
                waypointlist
            ):  # The curve only contains two segments
                suma_arc = sum(
                    [ar for ar in dist_xy[curve_index[i] - 1 : curve_index[i + 1]]]
                )  # We take the arc adding the previous and the next segments
            if curve_index[i + 1] - curve_index[i] == 1 and curve_index[
                i + 1
            ] + 1 >= len(
                waypointlist
...
def radius_profile_old(dtm_m, waypoints, scale_m_per_pix, min_radius):
    ...
    for p0, p1, p2 in zip(waypoints[:-2], waypoints[1:-1], waypoints[2:]):
        ...
        if angle > 1e-06:
            rad = (
                scale_m_per_pix / ((np.tan(angle / 2)))
                if angle <= np.pi / 2
                else (
                    scale_m_per_pix / ((np.tan((np.pi - angle) / 2)))
                    if angle < np.pi
                    else 94906265.2485031
                )
            )
...
def interactive_height_profile(interpolator, waypoints, scale_m_per_pix):
    ...
    diffe = [h1 - h0 for h0, h1 in zip(height_prof[:-1], height_prof[1:])]
    # print(f"------------------------------------")
...
def interactive_summary(waypoints, scale_m_per_pix, height_prof):
    ...
    summary["average_slope"] = (
        abs(height_prof_list[-1] - height_prof_list[0]) / summary["straight_distance"]
    )
    summary["average_acum_slope"] = (
        summary["total_acumulative_slope"] / summary["total_cumsum"]
    )
    track_quality, twist_num, turn_number_list = get_track_quality(
        waypoints, height_prof_list, scale_m_per_pix
    )
    summary["twist_number"] = twist_num
    summary["track_quality"] = track_quality
    print("SUMARY")

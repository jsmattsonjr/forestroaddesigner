...
def showMessage(message, msg_type="Info", level=Qgis.Info, duration=3):
    ...
    try:
        iface.messageBar().pushMessage(
            msg_type, message, level=level, duration=duration
        )
    except AttributeError:
...
def _road_options_label(slabel, r_options, w_road, cut, hc, fill, hf):
    ...
    if r_options:
        ...
        label += "_W_{}_C_{}_HC_{}_F_{}_HF_{}".format(
            w_road, cut_norm, hc, fill_norm, hf
        )
    return label
...
class BestPathFinder(object):
    ...
    def __init__(self, dtm_layer, exclusion_areas_layer=None):
        ...
        self.exclusion_areas = {
            ...
            "array": frd_utils.exclusion_areas_fn.create_exclusion_array(
                dtm_layer, exclusion_areas_layer
            ),
        }
    ...
    def add_segment_to(
        self, goal_coords, max_dist_m=None, iterative=False, force=False
    ):

        self.check_point(goal_coords)
        goal_index = self.converter.coord_to_index([goal_coords])
        self.optimizer.add_segment_to(goal_index[0], max_dist_m, iterative, force)
            ...
            if self.optimizer._waypoints_index == []:
                for elem in self.exclusion_areas["layer"].dataProvider().getFeatures():
                    if elem.geometry().contains(
                        ...
                        print(
                            f"Point at {goal_coords} lies within the exclusion zone!!!! Avoiding."
                        )
                        raise InvalidPointError
                    else:
    ...
    def _output_raw_filename(self):
        ...
        return os.path.join(
            ...
            self.RAW_FN_TEMPLATE.format(
                self._slopelabelcomplete(), self.current_output_index
            ),
        )
    ...
    def create_simplified_output_layer(self, polyline_threshold, show_graphics=False):
        ...
        if polyline_threshold > 0 and len(raw_path_index) > 0:
            ...
            simp_path_index = path_simplifier.from_threshold(polyline_threshold)
        else:
        ...
        summary, turn_number_list = viewers.summary(
            simp_path_index,
            self.converter.pixel_width,
            lines_height_data,
            slope_pen,
        ...
        slope_symbology.AttributeLayerSymbology(
            road_layer,
            "slope_p",
            "radius_m",
            "penalty",
...
class BestInteractivePathFinder(object):
    ...
    def _slope_label(self):
        return _interactive_label(
            self.parameters["inter_slope_pct"], self.parameters["inter_length"]
        )
    ...
    def _find_available_output_suffix(self):
        ...
        index = 1
        for template in (self.RAW_FN_TEMPLATE, self.SIMPLIFIED_FN_TEMPLATE):
            key_for_glob = os.path.join(self.output_folder, template.format("*"))
            # template.format(self._slope_label(), '*'))
    ...
    def search_segment_to(self, goal_coords):
        ...
        if self.interactive_check_point(goal_coords):
            ...
            goal_result, up_down = self.optimizer.interactive_search_segment_to_(
                goal_index[0]
            )
    ...
    def _output_raw_filename(self):
        ...
        """
        return os.path.join(
            self.output_folder, self.RAW_FN_TEMPLATE.format(self.current_output_index)
        )
    ...
    def create_simplified_output_layer(self, polyline_threshold, show_graphics=False):
        ...
        lines_height_data, lines_section_slope, lines_projected_cum_dist = (
            viewers.interactive_height_profile(
                self.optimizer.interpolator, simp_path_index, self.converter.pixel_width
            )
        )

        summary, turn_number_list = viewers.interactive_summary(
            simp_path_index,
            self.converter.pixel_width,
            lines_height_data,
        )
        ...
        # Add to the generated road layer a symbology that changes its color
        # according to its slope
        slope_symbology.InteractiveAttributeLayerSymbology(
            road_layer,
            "slope_p",
        )
        # Return both the raw, points layer and the simplified polyline layer
